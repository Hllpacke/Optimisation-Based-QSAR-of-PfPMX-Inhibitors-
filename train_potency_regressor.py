"""
Train a PMX potency regressor from a CSV file.
...
def parse_args() -> RegressionConfig:
    parser = argparse.ArgumentParser(
        description="Train a PMX corrected pIC50 regressor using RDKit descriptors and Morgan fingerprints."
    )
    parser.add_argument("--data-csv", required=True, help="Path to the training CSV placeholder, e.g. data/training.csv")
    parser.add_argument("--target-col", default="Corrected pIC50(D10)", help="Target column in the training CSV")
    ...
    parser.add_argument("--output-dir", default="results/potency", help="Directory for model, tables and plots")
...
def main() -> None:
    ...
    save_model_and_metadata(
        ...
        model_name="potency_model.joblib",
        metadata_name="potency_metadata.json",
    )
